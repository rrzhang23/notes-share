...
v4.0 及以下版本可通过 configure 和 cmake 两种方式安装。

### 分两种安装方式
configure 安装，v3.20.1 
```
# git show v3.20.1
# 21027a27c4c2ec1000859ccbcfff46d83b16e1ed
rm -rf third_party/benchmark/ third_party/googletest/ third_party/abseil-cpp/ third_party/jsoncpp/
git checkout 21027a2
make clean
git clean -dfx
cd third_party/
cp -r ../../benchmark/ .
cp -r ../../googletest/ .
cd benchmark/
git checkout 5b7683f
cd ../googletest/
git checkout 5ec7f0c
cd ../../

./autogen.sh
./configure --prefix=$HOME/local/protobuf-3.20.1 CXXFLAGS=-fPIC CFLAGS=-fPIC && make -j32 && make install
git checkout main

cd /home/zhangrongrong/local/protobuf-3.20.1
ln -s lib/ lib64
...
cd protobuf
# git show v22.1
# 839e8d25eab89b8cf5fa6a5a18575a8d383aaad1
rm -rf third_party/benchmark/ third_party/googletest/ third_party/abseil-cpp/ third_party/jsoncpp/
git checkout 839e8d25eab89b8cf5fa6a5a18575a8d383aaad1
make clean
git clean -dfx
cd third_party/
cp -r ../../abseil-cpp/ .
...
cp -r ../../googletest/ .
cd abseil-cpp/
git checkout 78be636
cd ../googletest/
git checkout 4c9a3bb
cd ../jsoncpp/
git checkout 9059f5c
cd ../../

mkdir build && cd build
cmake -DCMAKE_INSTALL_PREFIX=/home/zhangrongrong/local/protobuf-v22.1 ..
make -j32 && make install
cd ..
rm -rf build/
...
or
~~~
wget https://gitee.com/mirrors/gflags/repository/archive/v2.2.2.zip
mv v2.2.2.zip gflags-2.2.2.zip
unzip gflags-2.2.2.zip > /dev/null
cd gflags/
~~~
...
mkdir build && cd build
rm -rf ../build/*
cmake -DCMAKE_INSTALL_PREFIX=/home/zhangrongrong/.local/gflags-v2.2.2 -DBUILD_SHARED_LIBS=1 -DBUILD_STATIC_LIBS=1 -DGFLAGS_NAMESPACE=google ..
make -j32 && make install
~~~
...
cd googletest
mkdir build && cd build
cmake                        -DCMAKE_INSTALL_PREFIX=/home/zhangrongrong/.local/googletest-1.11.0 .. && make -j32 && make install
...
git clone git@github.com:google/googletest.git
cd benchmark
git checkout 7d0d906
cd ../googletest
git checkout 662fe38
cd ../../
~~~


`vim CMakeLists.txt` 加入：
~~~
...
```
mkdir build && cd build
cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/home/zhangrongrong/.local/leveldb-1.23 .. && cmake --build . -j8
cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/home/zhangrongrong/.local/leveldb-1.23  -DBUILD_SHARED_LIBS=ON .. && cmake --build . -j8
make install
```
...
### 旧的同时安装动态和静态库的方法：  
第三百行左右，`add_subdirectory("third_party/googletest")` 改成：
~~~
...
2.
默认只能编译出静态库，但是加入`-DBUILD_SHARED_LIBS=ON` 想编出动态库时，又只能编译出动态库。  
解决方法：
