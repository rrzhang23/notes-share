...
`fcntl-linux.h : fallocate()`  

`unistd : unlink()、close()、lseek()、ftruncate()、fsync()、pread()、pwrite()、close()`
...
### 3.库函数参数打包方案
详见：[库函数参数打包方案](http://note.youdao.com/noteshare?id=d574ddf4d577a0bafa32430381f073c1)

总结：  
stat 不难打包；  
fdopen 用于临时文件，暂不处理；  
readdir、opendir、closedir 不参照上面链接里的设想，而是全部放到远程文件 server 端处理。
